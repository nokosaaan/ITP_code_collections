...
vector<int> alpha(char &s, vector<int> vec){
    ...
    else if(s=='p'||s=='P') vec[15]+=1;
    else if(s=='q'||s=='Q') vec[16]+=1;
...
int main(){
    ...
    while(true){
        ...
        if(s=="z") a=true;
        else if(s[(s.size())-3]=='e'&&s[(s.size())-2]=='n'&&s[(s.size())-1]=='.') a=true;
        else if(s[(s.size())-1]=='=') a=true;
        else if(s[(s.size())-3]=='n'&&s[(s.size())-2]=='t'&&s[(s.size())-1]=='.') a=true;
...
/*感想：難しい、…笑いたけりゃ笑えよ、このコードを見て。
        本当はこんなコード作りたくなかったんだ！！！
        だけど仕方ないじゃないか！！！終了条件がわからなかったんだから！！
        データをカンニングして末尾の単語の組み合わせで無理やり終了させることに
        したけどマジで意味わからん過ぎて泣いちゃった。
        誰か最適解知ってたら教えてくださいマジで*/